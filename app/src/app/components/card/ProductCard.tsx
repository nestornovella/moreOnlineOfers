...
function ProductCard({ product }: Props) {
    ...
    return (
        <Link href={`/detail/${id}`}>
            <div style={{ fontFamily: "var(--font-roboto)" }} className="animate-card border rounded-xl p-2 text-center lg:text-start bg-white grid lg:grid-cols-2 relative">
                ...
                <div className="flex justify-between  items-center flex-col lg:py-6  ">
                    <h2 className="font-bold text-sm md:text-sm  text-gray-500">{name[0].toUpperCase() + name.slice(1)}</h2>
                    <div className="p-2 text-xs md:text-sm  text-gray-500">
                        <p>toque el producto que desea agregar al carrito</p>
                    </div>
                    <div className=""><div className="bg-[--rosado] text-white font-semibold py-1 px-7 -mx-4 rounded-lg w-fit relative top-4 -rotate-12">
                        <p className="text-xs">peso</p>
                        <h2>{`${measureValue + ' ' + measureUnits}`}</h2>
                    </div>
                        <div className="bg-[#042698] text-white font-semibold py-1 px-7 mx-4 rounded-lg w-fit pt-2 " >
