...
function ParenProductModal({cb}:{cb:(id:string)=>void}) {
    ...
    return (
        <div onClick={e => e.stopPropagation()} className="md:w-[50vw]  flex flex-col gap-2 px-4 text-white   bg-black relative z-[1110] p-2 rounded-xl">
...
function ProductCardMenu({ product, cb }: { product: ProductIF, cb:(id:string)=>void }) {
    ...
    return (
        <div className="w-full flex border p-1 rounded-xl gap-2 items-center font-semibold">
            <Image className="size-[80px] rounded-2xl object-cover" src={`${product.image}`} width={300} quality={70} height={300} alt="" />
            <div onClick={() => cb(product.id)} className="border w-full px-2">
                <h2>{product.name}</h2>
            </div>
        </div>
    )
